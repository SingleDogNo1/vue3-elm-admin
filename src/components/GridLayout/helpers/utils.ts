 ...
 */
export function getLayoutItem(layout: Layout, id: string): void | LayoutItem {
  for (let i = 0, len = layout.length; i < len; i++) {
 ...
 */
export function getFirstCollision(layout: Layout, layoutItem: LayoutItem): void | LayoutItem {
  for (let i = 0, len = layout.length; i < len; i++) {
...
export function validateLayout(layout: Layout, contextName = 'Layout'): void {
  ...
  const keyArr: Array<LayoutItem['i']> = []
...
/**
 * Convert a JS object to CSS string. Similar to React output of CSS.
 * @param obj
 * @returns {string}
 */
...
export function addPx(name, value) {
  if (typeof value === 'number' && !IS_UNIT_LESS[name]) {
    return value + 'px'
  } else {
    return value
  }
